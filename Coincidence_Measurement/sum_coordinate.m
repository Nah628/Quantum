...
minusCoordinate = zeros(height, width);
num_pixels = height*width;
for idx = 1:num_pixels
    currentMap = intensityCorr_all(:,:,idx);
    padded_currentMap = padarray(currentMap, [height, width], 0, 'both');

    % 相関マップの最大値
    max_value = max(padded_currentMap, [], 'all');
    [y_max, x_max] = find(padded_currentMap == max_value);

    % 最大値を持つ画素を中心に、200×200画素を切り出す
    tmp = padded_currentMap(y_max-height/2:y_max+height/2-1, x_max-width/2:x_max+width/2-1);

    % minus-coordinate用配列を更新
    minusCoordinate = minusCoordinate + tmp;

end

center = height/2+1;
minusCoordinate(center,:) =  (minusCoordinate(center-1,:) + minusCoordinate(center+1,:))/2;

% 正規化
normalized_minusCoordinate = (minusCoordinate - min(minusCoordinate(:))) / (max(minusCoordinate(:))-min(minusCoordinate(:)));

% 結果の表示
figure(1);
imagesc(normalized_minusCoordinate);
axis equal tight
axis xy
