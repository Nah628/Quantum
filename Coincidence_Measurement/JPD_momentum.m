%% Joint probability distribution - momentum
...
height = size(original_array,1);
width = size(original_array,2);
...
for dataSetNumber = 1:dataSet
    ...
    for x1 = 1:width
        ...
        temp_1 = sum(original_array(y2,:,1:N-1) .* self_signal,3);
        temp_2 = sum(original_array(y2,:,2:N) .* self_signal,3);
...
axis xy;
title(fprintf('joint probability distribution'));
xlabel('X1');
ylabel('X2');
colorbar;
